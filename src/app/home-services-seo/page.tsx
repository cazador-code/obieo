...
// --- Editorial Components (matching GEO guide style) ---
...
function DataCard({
  ...
  return (
    <div
      className={`p-5 md:p-6 rounded-xl border ${variantStyles[variant]} transition-transform hover:scale-[1.02]`}
    >
      <p className="text-sm text-[var(--text-muted)] mb-2 font-medium uppercase tracking-wide">
        {label}
      </p>
      <p
        className={`text-2xl md:text-3xl font-bold font-[family-name:var(--font-display)] ${valueStyles[variant]}`}
      >
        {value}
      </p>
      {subtext && (
        <p className="text-sm text-[var(--text-secondary)] mt-2">{subtext}</p>
      )}
    </div>
  )
}
...
function Paragraph({
  ...
  return (
    <p
      className={`${lead ? 'text-xl md:text-2xl leading-relaxed' : 'text-lg leading-relaxed'} text-[var(--text-secondary)] mb-6`}
    >
      {children}
    </p>
  )
